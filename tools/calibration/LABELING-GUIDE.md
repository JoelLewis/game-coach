...
Judge practical value, not raw centipawns: the same -1.5 swing is a mistake at 0.0 and nearly irrelevant at +9. A "missed win" counts as an error only when the win was actually lost or seriously reduced: had a winning shot, played a quiet move and ended up merely equal or slightly better -> usually 2, or 3 if the missed shot was a short forced mate or won a queen. If the position is STILL completely winning after the move (say +5 or better), missing a faster win or a forced mate is severity 0 with `missedTactic: true`: nothing practical was lost, and the pattern is still flagged for review.
